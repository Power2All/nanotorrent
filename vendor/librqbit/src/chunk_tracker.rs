...
impl ChunkTracker {
    ...
    /// Queued pieces, each paired with the RANK of the file that yielded it -
    /// its position in `file_priorities`, so 0 is the most wanted file.
    ///
    /// NanoTorrent addition. The rank was previously implicit in the iteration
    /// order, which was enough while that order was the whole selection
    /// strategy. Rarest-first reorders within a rank and must not reorder
    /// across one, so the caller now needs to see it.
    ///
    /// The rank is non-decreasing over the iteration, which is what lets the
    /// picker stop early instead of draining the whole queue.
    pub(crate) fn iter_queued_pieces_ranked<'a>(
        &'a self,
        file_priorities: &'a FilePriorities,
        file_infos: &'a FileInfos,
    ) -> impl Iterator<Item = (usize, ValidPieceIndex)> + 'a {
        file_priorities
            .iter()
            .enumerate()
            .filter_map(|(rank, p)| Some((rank, *p, file_infos.get(*p)?)))
            .filter(|(_rank, id, f)| self.per_file_bytes[*id] != f.len)
            .flat_map(|(rank, _id, f)| f.iter_piece_priorities().map(move |i| (rank, i)))
            .filter(|(_rank, id)| self.queue_pieces[*id])
            .filter_map(|(rank, id)| Some((rank, id.try_into().ok()?)))
            .filter_map(|(rank, id)| Some((rank, self.lengths.validate_piece_index(id)?)))
    }
