    ...
    pub fn per_peer_have_pieces(&self) -> Vec<(std::net::SocketAddr, u64)> {
        self.peers
            ...
            .filter_map(|entry| match entry.value().get_state() {
                PeerState::Live(live) => {
                    Some((*entry.key(), live.bitfield.count_ones() as u64))
                }
                _ => None,
            })
            .collect()
...
impl PeerConnectionHandler for &PeerHandler {
    ...
    fn update_my_extended_handshake(
        ...
        // Anonymous mode: don't advertise the client version so peers can't
        // fingerprint us by it.
